...
;REGISTER NAME TYPEOUT INITIALIZATION
       CLR    COMMNUM  ;START AT BEGINNING OF THE REGISTER NAME LIST
...
COPY1  LDAA   ,X       ;GET BYTE FROM SOURCE
       ...
       JMP    NOMORE   ;SHOULD BE NO MORE ON THE INPUT LINE
...
CKSUM1 INX             ;START OF CHECKSUM LOOP
       ADDA   Z        ;UPDATE CHECKSUM IN ACCA WITH BYTE POINTED TO
...
;NBRFMT - NUMBER OF CHARS THAT MATCH SO FAR IN THE MATCHING PROCESS
...
;DON'T ACCEPT OVER 6 BYTES
       ...
       STAA   ,X       ;SAVE BYTE
;                       MOVE BYTE POINTER TO NEXT LOCATION IN STRING
       STX    BYTPR    ;SAVE IT
       BRA    SEARC1
...
;MAKE USE OF INOUT LINE CHARACTER FETCH & COMMAND LIST CHAR FETCH ROUTINES

;INITIALIZE BYE POINTER TO START OF BYTE STRING
SEARC3 LDX    #BYTSTR-1
       STX    LISPTR

       CLR    NBRFMT   ;SET "NUMBER OF BYTES THAT MATCHED" TO ZERO
...
CMD3   LDX    SYNPTR
       ...
       CLR    NUMBAT   ;CLEAR NUMBER OF CHARACTERS MATCHED
...
;======================================================
;THIS ROUTINE TYPES OF COMMAND NUMBER "COMMNUM"
...
TYPCMD STX    XTEMP
       ...
       LDAA   COMMNUM  ;GET COMMAND NUMBER
...
;DETERMINE INOUT BASE & GO TO RIGHT ROUTINE
       LDAA   IBCODE
       CMPA   #1
       BEQ    HEXNUM
...
;======================================================
;THIS ROUTINE GETS THE NECT CHARACTER FROM THE INPUT LINE BUFFER
;ACCA IS PRESERVED
...
; THE INPUT LINE IS STORED BEGINNING AT THE ADDRESS STORED IN BUFREG
...
GETLIN LDX    BUFREG   ;SET POINTER TO ONE LESS THAN THE BEGINNING OF THE LINE BUFFER
...
OUTCH2 STAA   ,X       ;SAVE OUTPUT ADDRESS
...
MSGNVE ASC    "CHECKSM ERROR " ; FOR VERIFY & LOAD COMMANDS
       DB     4

MSGCCL ASC    "CAN'T CLEAR" ; TEST COMMAND
       DB     4
...
RESADR JMP    START
;*****
SWIADR STS    SP       ;SACE STACK POINTER OF PROGRAM BEING DEBUGGED
       JMP    X
;*****
...
;**************************************************
       ...
       JMP    TYPCMD   ;TYPES OUT COMMAND NUMBER "COMMNUM" IN LIST ACCA
